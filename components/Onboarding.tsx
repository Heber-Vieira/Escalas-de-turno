...
export const Onboarding: React.FC<OnboardingProps> = ({ onComplete, onCancel, isAddingExtra, existingRoles = [] }) => {
  ...
  const toggleOffDay = (day: number) => {
    ...
    if (current.includes(day)) {
      setConfig({ ...config, offDays: current.filter(d => d !== day) });
    } else {
  ...
  const steps = [
    {
      ...
      content: (
        <div className="space-y-4">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest text-center">Como podemos te chamar?</p>
            <input 
            autoFocus
            className="w-full p-4 text-2xl font-black border-b-4 border-emerald-500 outline-none bg-transparent text-center text-gray-900"
    ...
    },
    {
      ...
      content: (
        <div className="space-y-6">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest text-center">Qual sua função na equipe?</p>
            <input 
              autoFocus
              className="w-full p-4 text-2xl font-black border-b-4 border-purple-500 outline-none bg-transparent text-center text-gray-900"
              placeholder="Ex: Operador, Mecânico, Soldador..."
              value={config.role}
        ...
        <div className="grid grid-cols-1 gap-2 max-h-[40vh] overflow-y-auto no-scrollbar pr-1">
          {Object.values(ShiftType).map(type => (
            <button
              ...
              className={`p-4 rounded-2xl text-left border-2 transition-all ${config.shiftType === type ? 'border-blue-500 bg-blue-50 text-blue-700 ring-4 ring-blue-500/10' : 'border-gray-100 hover:border-blue-200 text-gray-700'}`}
            >
        ...
        <div className="grid grid-cols-1 gap-3">
          {[
            ...
            <button
              ...
              className={`p-4 rounded-2xl flex items-center gap-4 border-2 transition-all ${config.turn === t.id ? 'border-orange-500 bg-orange-50 text-orange-700 ring-4 ring-orange-500/10' : 'border-gray-100 hover:border-orange-200 text-gray-700'}`}
            >
              <div className={config.turn === t.id ? 'text-orange-500' : 'text-gray-300'}>{t.icon}</div>
      ...
      content: (
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2 text-center">
                    ...
                    <input type="number" className="w-full p-4 rounded-2xl border-2 border-indigo-100 text-center font-black bg-white text-gray-900" value={config.rotatingWorkDays} onChange={e => setConfig({...config, rotatingWorkDays: Math.max(1, parseInt(e.target.value))})} />
                </div>
                <div className="space-y-2 text-center">
                    <label className="text-[10px] font-black opacity-40 uppercase">Dias Folga</label>
                    <input type="number" className="w-full p-4 rounded-2xl border-2 border-indigo-100 text-center font-black bg-white text-gray-900" value={config.rotatingOffDays} onChange={e => setConfig({...config, rotatingOffDays: Math.max(1, parseInt(e.target.value))})} />
                </div>
              </div>
          ...
          <div className="space-y-2">
            ...
            <input 
              type="date"
              className="w-full p-4 rounded-2xl border-2 border-indigo-100 text-center font-bold bg-white text-gray-900 outline-none focus:border-indigo-500 transition-all [color-scheme:light]"
    ...
    <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center p-6 bg-white overflow-hidden">
      ...
      >
        <div className="flex flex-col items-center text-center space-y-4">
          <div className="p-5 bg-gray-50 rounded-[30px] shadow-inner">{currentStepData.icon}</div>
          <h1 className="text-xs font-black tracking-[0.3em] text-gray-400 uppercase">{currentStepData.title}</h1>
        </div>
        ...
        <div className="flex items-center justify-between pt-8">
          <div className="flex items-center gap-4">
            {step > 1 ? (
              <button onClick={prev} className="p-3 text-gray-300 hover:text-gray-600 transition-colors"><ArrowLeft size={24} /></button>
            ) : isAddingExtra && (
               <button onClick={onCancel} className="p-3 text-red-400 font-black uppercase text-[10px] tracking-widest">Cancelar</button>
            )}
            <div className="flex gap-1.5">
              {steps.map((_, i) => (
                <div key={i} className={`h-1.5 rounded-full transition-all ${i < step ? 'w-6 bg-pink-500' : 'w-2 bg-gray-100'}`} />
              ))}
            </div>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-2 px-8 py-4 bg-gray-900 text-white rounded-full font-black text-xs uppercase tracking-widest shadow-2xl disabled:opacity-20 transition-all hover:scale-105 active:scale-95"
          >
