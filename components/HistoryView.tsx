                    ...
                    <div className="space-y-4 relative">
                        ...
                        <AnimatePresence mode="popLayout">
                            {allEvents.map((event, idx) => {
                                ...
                                const validDate = isValid(eventDate) ? eventDate : new Date();
                                ...
                                >
                                    <div className={`w-12 h-12 flex-shrink-0 rounded-2xl flex flex-col items-center justify-center shadow-sm border ${isSameMonth(validDate, new Date()) ? `${event.color} border-white/20 text-white` : 'bg-white border-gray-100 text-gray-400'}`}>
                                        <span className="text-[14px] font-black leading-none">{format(validDate, 'dd')}</span>
                                        <span className="text-[7px] font-black uppercase tracking-tighter opacity-70">{format(validDate, 'MMM', { locale: ptBR })}</span>
                                    </div>
                                    <div className="flex-1 bg-white border border-gray-100 p-3.5 rounded-[24px] shadow-sm flex items-center justify-between group">
                                        <div className="flex-1 min-w-0 pr-4">
                                            ...
                                            <h4 className="font-bold text-gray-800 text-[11px] truncate">
                                                {event.type === 'absence' ? event.title : format(validDate, 'EEEE', { locale: ptBR })}
                                            </h4>
